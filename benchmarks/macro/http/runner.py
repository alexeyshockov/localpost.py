...
For each (python, stack, scenario) cell:
  1. Boot the stack as a subprocess (``<python> -m benchmarks.macro.http.apps.<stack>``).
...
from benchmarks.macro.http.scenarios import SCENARIOS
from benchmarks.macro.http.stacks import DIM_KEYS, GROUPS, STACKS
...
def main() -> int:
    return entrypoint(
        ...
        stacks=STACKS,
        groups=GROUPS,
        apps_pkg="benchmarks.macro.http.apps",
