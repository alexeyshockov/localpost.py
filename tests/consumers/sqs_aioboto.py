...
from localpost.consumers.sqs import SqsMessage, SqsMessages, sqs_queue_consumer, AsyncConsumerClient
...
async def test_happy_path(local_sqs):
    ...
    @sqs_queue_consumer(lambda: AsyncConsumerClient.create(
        queue_url, get_aio_session().create_client("sqs", **local_sqs)))
    @flow.handler
