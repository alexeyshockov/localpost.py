...
async def thread_pool_handler(inner: RequestHandler, /) -> AsyncGenerator[RequestHandler]:
    """Async context manager: yields a ``RequestHandler`` that runs each
    request on a worker thread with a *borrowed* conn.
    ...
    Routing-only handlers (no body, no I/O) can run on the selector by
    not wrapping the router with this. The Router's 404/405 path
    completes inline via ``ctx.complete``; matched routes hit the
    per-route handler — wrap *that* with the pool, or wrap the whole
    router. Whichever shape the user composes is what runs.
    ...
    async with _pool_context() as pool:
        yield pool.dispatch(inner)


# Back-compat alias: the old streaming_pool_handler took a
# ``Callable[[_NativeReqCtx], None]`` and dispatched on a borrowed conn
# without pre-buffering body. After unifying the dispatch shape that's
# exactly what thread_pool_handler does, so streaming_pool_handler is
# now an alias for it.
